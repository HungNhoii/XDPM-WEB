...
if ($vAct == 'del' && !empty($vID)) {
	...
	$vResult_Del = $o->query($vSQL_Del)->fetchAll();
...
if (isset($_POST["btnSubmit"])) {
	if ($vAct == 'add' || $vAct == 'edit') {
		...
		if ($vAct == 'add') {
			...
			$vResult_Ins = $o->query($vSQL_Ins);

			if ($vResult_Ins) {
				//Lấy ID mới nhất sau khi thêm mới
				$vSQL_Role_Sel = "SELECT MAX(ID) as MenuID FROM menus";
				$vResult_Role_Sel = $o->query($vSQL_Role_Sel)->fetchAll();
				$rowMenuID = $vResult_Role_Sel[0];
...
if ($vAct == 'edit') {
	...
	$vResult_Edit = $o->query($vSQL_Edit)->fetchAll();
	$vRow_Edit = $vResult_Edit[0];
	...
	$vResult_Edit_Role = $o->query($vSQL_Edit_Role)->fetchAll();
	while ($vRow_Edit_Role = $vResult_Edit_Role[0]) {
...
$vResult_Sel = $o->query($vSQL_Sel)->fetchAll();

//Lấy danh sách vai trò
$vSQL_Roles = "SELECT * FROM roles ORDER BY RoleName ASC";
$vResult_Roles = $o->query($vSQL_Roles)->fetchAll();
...
<fieldset>
    ...
    <form method="post" action="" id="mmenus_Form" enctype="multipart/form-data">
        <table border="0" cellspacing="0" cellpadding="5">
            ...
            <tr>
                <td>Tên menu:</td>
                <td><input type="text" name="txtMenuName" id="txtMenuName" style="width:250px;"
                        value="<?php echo isset($eMenuName) ? $eMenuName : ''; ?>" /></td>
            </tr>
            <tr>
                <td>Đường dẫn:</td>
                <td><input type="text" name="txtPath" id="txtPath" style="width:250px;"
                        value="<?php echo isset($ePath) ? $ePath : ''; ?>" /></td>
            </tr>
            <tr>
                <td>Cho phép hiển thị:</td>
                <td><input type="checkbox" name="chkVisible" id="chkVisible"
                        <?php echo $eVisible == 1 ? 'checked="checked"' : ''; ?> /></td>
            </tr>
            <tr>
                <td>Vị trí:</td>
                <td>
            ...
            <tr>
                ...
                <td>
                    <?php
						foreach ($vResult_Roles as $eRoles) {
							$checkedRole = '';
...
<fieldset>
    ...
    <table border="0" cellspacing="0" cellpadding="0" class="table-result">
        ...
        <?php
		$stt = 1;
		foreach ($vResult_Sel as $vRowSel) { ?>
        <tr>
            ...
            <td align="center" width="50"><a
                    href="?mod=<?php echo $vModule; ?>&act=edit&id=<?php echo $vRowSel["ID"]; ?>">Sửa</a></td>
            <td align="center" width="50"><a
                    href="?mod=<?php echo $vModule; ?>&act=del&id=<?php echo $vRowSel["ID"]; ?>"
                    onclick="return confirm('Bạn có muốn thực hiện xóa bản ghi này không?')">Xóa</a></td>
        </tr>
        <?php
			$stt++;
		} ?>
